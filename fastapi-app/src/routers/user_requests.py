...
from src.services.user_requests import get_user_requests
...
@router.get("/requests/{request_id}", response_model=RequestModelNoLike)
...
async def edit_request(request_id: int, request_data: RequestUpdate,
    ...
    if request_data.categories is not None:  # Check if categories are provided in the request
        request_categories = []
